            ...
            <div class="col-lg-offset-3 col-lg-6">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h3 class="panel-title">Ajax List <a href="#" id="addNew" class="pull-right" data-toggle="modal" data-target="#myModal"><i class="fa fa-plus" aria-hidden="true"></i></a></h3>
                    </div>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.js"></script>
    ...
    $(document).ready(function () {
        $(document).on('click', '.our-item', function () {
                var text = $(this).text();
        ...
        $('#addButton').click(function (event) {
            ...
            if (text == "") {
                alert('Plese type anything for item');
            } else {
                $.post('list', {'text': text, '_token': $('input[name=_token]').val()}, function (data) { // the [list] => routes name
                    console.log(data);
        ...
        $('#delete').click(function () {
           var id = $('#id').val();
            $.post('delete', {'id': id, '_token': $('input[name=_token]').val()}, function (data) { // the [list] => routes name
                console.log(data);
                $('#items').load(location.href + ' #items');
            });
        });
        ...
        $('#saveChanges').click(function () {
            ...
            $.post('update', {'id': id, 'value': value, '_token': $('input[name=_token]').val()}, function (data) { // the [list] => routes name
                console.log(data);
                $('#items').load(location.href + ' #items');
            });
        });
